...
- (void)viewDidLoad {
    ...
    _textView = [[LMWordView alloc] initWithFrame:self.view.bounds];
    ...
    NSAttributedString *content = [LMTextHTMLParser attStringFromAttributedString:attributedText textView:self.textView htmlString:_htmlString];
...
- (void)updateNavUI {
    ...
    [self.rightButton setTitle:@"保存修改" forState:(UIControlStateNormal)];
    self.rightButton.width = 80;
}

- (void)rightBarButtonClick {
    if (_uploadingImage && _uploadingImage.count > 0) {
        MBProgressHUD *hud = [MyTools createHudWithFrame:self.view];
        hud.label.text = @"正在保存修改...";
        [hud showAnimated:YES];
        __weak typeof(self) wSelf = self;
        [RACObserve(self, uploadingImage) subscribeNext:^(id  _Nullable x) {
            SLog(@"%@",x);
            if (!x) {
                [wSelf close];
                [wSelf closeHud:hud];
...
- (void)setUploadingImage:(NSMutableArray *)uploadingImage {
...
- (void)lm_imageSettingsController:(LMImageSettingsController *)viewController insertImage:(UIImage *)image {
    ...
    if (!_uploadingImage) {
        _uploadingImage = [NSMutableArray array];
    }
    [_uploadingImage addObject:degradedImage];
    
    __weak typeof(self) wSelf = self;
    [QNiuUploadFile uploadDataToQNiuWithImage:degradedImage success:^(NSString *key) {
        NSString *urlStr = [NSString stringWithFormat:@"%@%@",QiNiuBaseUrl,key];
        attachment.attachmentType = LMTextAttachmentTypeImage;
        attachment.userInfo = urlStr;
        [_uploadingImage removeObject:degradedImage];
        if (_uploadingImage.count == 0) {
            wSelf.uploadingImage = nil;
        }
    } fail:^{
        [_uploadingImage removeObject:degradedImage];
        if (_uploadingImage.count == 0) {
