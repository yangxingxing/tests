...
//menu弹出自定义view
- (UIView *)menu:(DOPDropDownMenu *)menu viewForDropDownMenuAtIndex:(NSInteger)index;
//menu弹出自定义view高度
- (CGFloat)menu:(DOPDropDownMenu *)menu heightForDropDownMenuAtIndex:(NSInteger)index;
//弹出之前先请求数据
- (BOOL)menu:(DOPDropDownMenu *)menu willDorpDownMenuAtIndex:(NSInteger)index success:(dispatch_block_t)success;
//  返回 menu 第column列 每行选中的image
- (NSString *)menu:(DOPDropDownMenu *)menu imageSelectedNameForRowAtIndexPath:(DOPIndexPath *)indexPath;
//是否显示右边的tableview
- (BOOL)menu:(DOPDropDownMenu *)menu itemsExistAtColumn:(NSInteger)column;
//是否刷新二级
- (BOOL)menu:(DOPDropDownMenu *)menu itemsExistAtColumn:(NSInteger)column row:(NSInteger)row;

//menu文字设置
- (NSString *)menu:(DOPDropDownMenu *)menu titleForMenuClickAtIndexPath:(DOPIndexPath *)indexPath;
...
//点击顶部menu按钮，执行自己的跳转动作 不弹出选择框
- (BOOL)menu:(DOPDropDownMenu *)menu didSelectedMenuButtonAtIndex:(NSInteger)index;
...
//@property (nonatomic, strong) NSMutableArray  *unShowMenuArray; // 保存需要手动跳转menu位置  0，1，2，3
@property (nonatomic, strong)NSMutableArray *selectedIndexPathAry;
@property (nonatomic, assign) BOOL isDateSelect;  //日期选择
