...
- (void)setupConstraints:(BOOL)isExist;
...
- (void)dzn_reloadEmptyDataSet
{
    ...
    if (([self dzn_shouldDisplay] && [self dzn_itemsCount] == 0) || [self dzn_shouldBeForcedToDisplay])
    {
        ...
        if (customView) {
            ...
            if ([self isButtonExist]) {  //添加代码 当button图片和字体同时存在时调整图片和字体的位置
            
                CGSize sizeTitle = [view.button.titleLabel sizeThatFits:CGSizeZero];
                CGFloat w = self.frame.size.width;
                CGFloat h = self.frame.size.height;
                CGFloat image_w = view.button.imageView.image.size.width;
                CGFloat image_h = view.button.imageView.image.size.height;
                
                
                view.button.contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
                view.button.contentVerticalAlignment   = UIControlContentVerticalAlignmentTop;
                
                view.button.imageEdgeInsets = UIEdgeInsetsMake((h - image_h) * 0.5, (w - image_w) * 0.5, 0, 0);
                view.button.titleEdgeInsets = UIEdgeInsetsMake((h + image_h) * 0.5 + 8,
                                                               -image_w + (w - sizeTitle.width) * 0.5, 0, 0);
            }
        }
        ...
        [view setupConstraints:[self isButtonExist]];
...
- (void)setupConstraints:(BOOL)isExist
{
    ...
    if (_customView) {
        ...
        if ([self canShowButton]) {
            ...
            //修改代码 调整button的边距为0
            [self.contentView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-0-[button(>=0)]-0-|"
        ...
        for (int i = 0; i < subviewStrings.count; i++) {
            ...
            if ([string isEqualToString:@"button"] && isExist) {  //添加代码 当button的图片和文字同时存在设置button的高度
                [verticalFormat appendFormat:@"[%@(==%f)]",string,self.frame.size.height];
            } else {
