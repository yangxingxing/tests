...
- (IBAction)keyboardViewAction:(UIButton *)sender
{
    ...
    switch (tag)
    {
        ...
        {
            // 小数点
            if(self.textFiled.text.length > 0 && ![self.textFiled.text containsString:@"."] && self.keyboardType == UIKeyboardTypeDecimalPad)
        ...
        default:
        {
            ...
            if (range.location != NSNotFound) {
                if (self.textFiled.text.length - range.location > _decimal) return;
            }
            if (self.textFiled.text.length >= self.maxLen) return;
            // 数字
            self.textFiled.text = [NSString stringWithFormat:@"%@%ld",self.textFiled.text,sender.tag - 1000];
//            [[NSNotificationCenter defaultCenter] postNotificationName:@"textFieldValueDidChangeNoticeFication" object:nil userInfo:@{@"textFieldValueDidChangeNoticeFication":self.textFiled}];
        }
            break;
    }
    if (_textDidChangeBlock) {
        _textDidChangeBlock(self.textFiled);
    }
}
