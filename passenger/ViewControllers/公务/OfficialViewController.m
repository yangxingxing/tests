...
- (void)initViews {
    _tableView = [MyTools createTableViewWithFrame:CGRectMake(0, 0, SCREENW, SCREENH-64-64*ScreenRate) delegate:self backgroundColor:UITableViewBackgroundColor rowHeight:145];
...
- (UIView *)waitReviewOrder {
    UIButtonCustom *waitReviewOrderView = [MyTools createButtonWithFrame:CGRectMake(10, 5, SCREENW - 2*10, 50*ScreenRate) title:nil titleColor:nil imageName:nil backgroundImageName:nil target:self selector:@selector(waitReviewOrderViewClcik:)];
    ...
    orderLabel.attributedText = [MyTools getDifferentFontAndColorWithString:@"有2个订单未审核" containStr:@"2" font:15 color:textRedColorPass];
...
// 用车申请
- (void)initBottomView {
    UIView *bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.height - 64*ScreenRate - 64, SCREENW, 64*ScreenRate)];
...
- (void)reviewbtnClick {
    PersonalViewController *peronal = [PersonalViewController new];
    peronal.useCarType = UseCarTypeOfficial;
    [self open:peronal];
}

- (void)waitReviewOrderViewClcik:(UIButtonCustom *)btn {
    AuditOrderViewController *audit = [AuditOrderViewController new];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *cellStr = @"OfficalCell";
